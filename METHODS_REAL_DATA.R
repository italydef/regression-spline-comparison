...
# ============================
RC<-function(yy,xx){
  Gmod <- NGeDS(yy ~ f(x), beta = 0.5, phi = 0.975)
  mu<-predict(Gmod, type="response")
  vscale<-sqrt(mean((yy-mu)^2))
...
EAPS<-function(yy,x){
  ...
  pred <- tryCatch({
    # ---- primary: adaptive-knot GAM (as before) --------------------------
  ...
  error = function(e) {
    # ---- fallback: mgcv adaptive smoother --------------------------------
    # fit_gam_optim can fail on series with sharp jumps (e.g. well-log): its
    # moving knots collapse onto a discontinuity, so a "cr" smooth ends up
    # with fewer distinct knots than k. bs = "ad" places its own basis and
    # is robust to that, so EAPS still returns an adaptive fit.
    message("  EAPS: adaptive-knot fit failed (", conditionMessage(e),
            ") - falling back to mgcv adaptive spline (bs = 'ad')")
    kfb <- min(40L, max(10L, round(nrow(my_data) / 20)))
    fb  <- gam(y ~ s(x1, bs = "ad", k = kfb), data = my_data, method = "REML")
    as.vector(predict(fb))
  })
...
FAPS<-function(yy,x){
  ...
  yfit<-asp2(yy~f(x,knots=kn,var.knots=kn.var,adap=TRUE),returnFit=TRUE)
  pred<-yfit$fitted
  return(pred)
 }
