...
#include "barsN_funcs1.h"
#include "barsN_utils.h"

/* see barsN_funcs.h for struct definitions */
...
Bars_PairedData *New_PairedData(int nd, int nf){
  Bars_PairedData *pd;
  pd = (Bars_PairedData *)calloc(1L,sizeof(Bars_PairedData));
  (*pd).n = nd;
  (*pd).nf = nf;
  (*pd).nc = nd;
  (*pd).x_raw = tssdvec(nd);
...
void Free_PairedData(Bars_PairedData *pd){
  free((*pd).x_raw);
  Free_X_Grid((*pd).xg);
  Free_X_Grid((*pd).fg);
  Free_X_Grid((*pd).cg);
...
/*  
    Using the knots in Model m, and data contained in PairedData pd,
...
*/
  ...
  int i,temp,j,tt,maxp,numx,lwork,nc,*cc,the_count,col,coltot,colcurr,rn;
  Bars_X_Grid *xg;
  double *basis,the_value,*extended_basis;

  if (fit_version == 1){
    xg = (*pd).fg;
    basis = (*m).basis_fine;
  } else {
    if ((*pd).use_collapsed == 1){
      xg = (*pd).cg;
      basis = tssdvec((*xg).n * ((*m).k + 4));
      extended_basis = (*m).basis;
    } else {
      xg = (*pd).xg;
      basis = (*m).basis;
    }
  ...
  *info = 0;
  ...
  if ((fit_version != 1) && ((*pd).use_collapsed == 1)){
    nc = (*pd).nc;
    cc = (*pd).ccount;
    temp = (*m).k + 4;
    rn = (*m).n;
    for(col=0;col<temp;col++){
      ...
      for(i=0;i<nc;i++){
    the_count = cc[i];
    the_value = basis[coltot + i];
    for(j=0;j<the_count;j++){
      extended_basis[colcurr++] = the_value;
    }
      }
    }
    free(basis);
...
void FitNormalModel(Bars_Model *m, 
         Bars_PairedData *pd, 
         Bars_WorkSpace *ws, double *condnum){

  double *g,*J,*theta,loglik1 = 0.0,
  anorm = 1.0, rcond= -1.0, *x, *y, eps_cond,sig;
  int info=88, p,p1,p2,i,iter,n,okay;
...
/* IMPORTANT: the first two basis columns are NOT to be used. */
  ...
  memcpy(g + n*p,y,sizeof(double)*n);
  /* calculate J = X'g = [X'X|X'Y] */
  dgemm(&Trans,&NoTrans,&p,&p1,&n,&One,x,&n,g,&n,&Zero,J,&p);
  anorm = getNorm(J,p);
  /* do Cholesky decomposition of X'X */
  /* X'X = U'U. The U matrix is stored in the upper
     triangle of J. The lower triangle remains unchanged,
     that is, it is the lower triangle of X'X */
  ...
  if (info != 0) {
    ...
    if ((info != 0) || (rcond < eps_cond)) {
      ...
      /* solve (X'X)B = X'Y for B. Solution is in the last column
     of J */
    ...
    dgemm(&NoTrans,&NoTrans,&n,&iOne,&p,&One,x,&n,
          (J + p2),&p,&Zero,(*m).fitted_values,&n);
    /* store Cholesky decomposition of X'X for
       later use when we want Cov(beta hat) */
    memcpy((*m).J,J,(sizeof(double)*p*p1));

    sig = 1.0;
    loglik1 = normal_loglik(n,p,(*m).fitted_values,y,&sig);
    (*m).sigma = sig;
...
void SetBasisAndFitModel(Bars_Model *m, 
         ...
         Bars_WorkSpace *ws, double *condnum){
    int info;
  SetModelBasis(m,pd,ws,0,&info);
  if (info == 0){
    FitNormalModel(m,pd,ws,condnum);
  } else {
    *condnum = -1.0;
    (*m).fit_info = 0;
  }
...
void RandBeta(Bars_Model *m,Bars_PairedData *pd, Bars_BarsParams *bp,
  ...
  xb(n,p,(*m).basis + 2 * n,lastbeta,
      (*m).random_mu);
  lastfi = normal_loglik(n,p,(*m).random_mu,(*pd).y,&sig);
  ...
  for(MHi=0;MHi<MHiters;MHi++){
    ...
    do{
      ...
      /* solve (X'X)^(1/2) A = z for A. 
     So A = (X'X)^(-1/2)z ~ N(0,(X'X)^(-1)) 
     Solution is placed in z */
      dtrtrs(&Upper,&NoTrans,&NoUnit,&p,&iOne,J,&p,curbeta,&p,&info);
    } while((info != 0) && (iters < 20));
    if ((iters == 20) && (info != 0)) {
      printf("Maximum number of beta iterations reached.\n");
      exit(1);
    }
    /* z = sigma*z + beta_hat ~ N(beta_hat,sigma^2 * (X'X)^(-1)) */
    curhi *= -0.5;
    affine(p,(*m).sigma,curbeta,(J + p*p));
    xb(n,p,(*m).basis + 2 * n,curbeta,
    (*m).random_mu);
    curfi = normal_loglik(n,p,(*m).random_mu,(*pd).y,&sig);
...
void RandMu(Bars_Model *m, Bars_PairedData *pd, Bars_BarsParams *bp, 
        int *laterej,int iternum){
  /* Generate random beta, and calculate mu = X*beta
     for both the design basis and the grid basis */
  RandBeta(m,pd,bp,laterej,iternum);
  xb((*m).n,(*m).k + 2,(*m).basis + 2 * (*m).n,(*m).beta,
      (*m).random_mu);
  xb((*m).nf,(*m).k + 2,(*m).basis_fine + 2 * (*m).nf,(*m).beta,
      (*m).random_mu_fine);
}

void Full_BIC(Bars_Model *m, Bars_PairedData *pd){
/* 
...
*/
  ...
  w = (double)((*m).k) + 1.5;
  p = (*m).k + 2;
  v = normal_loglik((*m).n,p,(*m).random_mu,(*pd).y,&tt);
...
void FormFirstKnots(Bars_Model *m, Bars_BarsParams *bp, Bars_PairedData *pd,Bars_PriorParams *pp){
  /* PairedData and PriorParams are inputs that are currently not needed.
     They may be useful if some alternative initial knot scheme is made 
     available, such as logspline for barsP, the Poisson version. 
     */
...
void saveModelStats(Bars_Model *m, Bars_PairedData *pd, 
...
/*
  get_mode_stats(nf,(*((*bd).fg)).x,(*m).random_mu_fine,(*bd).x_rawmin, 
         (*bd).x_rawmax, z);
*/
  get_mode_stats0(nf,(*((*pd).fg)).x,(*m).random_mu_fine,z);
  max_spline(nf,(*((*pd).fg)).x,(*m).random_mu_fine,z,uf,df,wf,pf);
  z[0] *= (*pd).x_rawmax - (*pd).x_rawmin;
  z[0] += (*pd).x_rawmin;
  z[2] = (double)((*m).k);
...
void saveBIC(Bars_Model *m, Bars_PairedData *pd, Bars_SampStat *ss, int simnum){
...
void saveSampStatParams(Bars_Model *m, Bars_PairedData *pd,
            Bars_SampStat *ss, int simnum, double *z){
  int n,nf;
  saveModelStats(m,pd,ss,simnum,z);
  saveBIC(m,pd,ss,simnum);
  n = (*m).n;
  nf = (*m).nf;
...
void CheckFirstFit(Bars_Model *m, Bars_PairedData *pd,
           Bars_PriorParams *pp,
           Bars_WorkSpace *ws){
  ...
  if ((*m).fit_info == 0){
    ...
    SetKnotsAll(m);
    SetBasisAndFitModel(m,pd,ws,&condnum);
    if (condnum > maxcn) {
      cnk = i;
      maxcn = condnum;
    }
...
void SetBirthModel(Bars_Model *m1, Bars_Model *m2, Bars_PairedData *pd, 
         Bars_BarsParams *bp, Bars_WorkSpace *ws){
  int k,k_rand;
...
void SetDeathModelBE(Bars_Model *m1, Bars_Model *m2, Bars_PairedData *pd, 
         Bars_BarsParams *bp, Bars_WorkSpace *ws){
  int k,dk_pos,best_i,i;
  double tau,*knots,cand,dens,best_loglike,condnum = -1.0;
  ...
  tau = (*bp).tau;
  for(i=0;i<k;i++){
    SetDeathModelKnots(i,k,knots,m2);
    SetBasisAndFitModel(m2,pd,ws,&condnum);
    if ((*m2).fit_info > 0){
        if(best_i<0){
            best_loglike = (*m2).loglikelihood;
            best_i = i;
        } else {
            if((*m2).loglikelihood > best_loglike){
                best_loglike = (*m2).loglikelihood;
                best_i = i;
            }
        }
...
void SetRelocateModel(Bars_Model *m1, Bars_Model *m2, 
              Bars_PairedData *pd, Bars_BarsParams *bp, 
              Bars_WorkSpace *ws){
  int k,dk_pos;
...
void write_iter_params(Bars_BarsParams *bp, Bars_Model *m, Bars_PairedData *pd, int np, 
...
void write_summ_params (Bars_BarsParams *bp, Bars_PairedData *pd,
            Bars_SampStat *ss,
  ...
  Bars_X_Grid *xg;
  if ((*bp).use_summ_mu){
    write_vector((*bp).summ_mu_file,(*pd).x_raw,n);
...
void Bars_MCMC(Bars_Model *m1, Bars_Model *m2, 
  ...
  double *birth_probs,*death_probs,u,*z,trial_bic;
  ...
  /* set starting knots using evenly spaced
     knots. A user-defined parameter
     could permit specification of
     an alternative scheme, such as
     logspline for barsP, the Poisson version.
  */

  FormFirstKnots(m_old,bp,pd,pp);

    u = -1.0;
    SetBasisAndFitModel(m_old,pd,ws,&u);
    CheckFirstFit(m_old,pd,pp,ws);
    trial_bic = 1.0;
    while(((*m_old).k > 1) && (trial_bic > 0)){
        SetDeathModelBE(m_old,m_new,pd,bp,ws);
        trial_bic = (*m_new).loglikelihood - (*m_old).loglikelihood + 0.5*log((*m_old).n);
        if(trial_bic > 0){
            m_temp = m_old;
            m_old = m_new;
            m_new = m_temp;
  ...
  for(i=0;i<tot_iter;i++){
    ...
    if (u <= birth_probs[k]){
      /* Birth Step */
      bdr = 0;
      SetBirthModel(m_old,m_new,pd,bp,ws);
    } else if ((1.0 - u) < death_probs[k]){
      /* Death Step */
      bdr = 1;
      SetDeathModel(m_old,m_new,pd,bp,ws);
    } else {
      /* Relocation Step */
      bdr = 2;
      SetRelocateModel(m_old,m_new,pd,bp,ws);
    }
    moves[bdr]++;
    moves[3]++;
    u = (double) genunf(0.0,1.0);
    ...
    if (i >= burn_iter){
      SetFineBasis(m_old,pd,ws);
      late_rejection = 0;
      RandMu(m_old,pd,bp,&late_rejection,i);
      if (late_rejection){
    i--;
      } else {
    saveSampStatParams(m_old,pd,ss,i-burn_iter,z);
    write_iter_params(bp,m_old,pd,np,z,i);
      }
    }
  }
  ...
  CloseFiles(bp,1,0);
  OpenFiles(bp,0,1);
  write_summ_params(bp,pd,ss,os,n,nf);
  CloseFiles(bp,0,1);
