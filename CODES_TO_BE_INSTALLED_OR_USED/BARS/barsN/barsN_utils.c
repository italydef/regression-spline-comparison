...
utility functions for barsN: Bars for Normal Data
...
void normalize(int n, double *xin, double *xout, double *xmin, double *xmax){
  ...
  for(i = 1;i<n;i++){
    if((xout[i] + DBL_EPSILON) <= xout[i-1]){
      printf("X values must be strictly increasing.\n");
      printf("x[%i] = %1.20le, x[%i] = %1.20le\n",i-1,xin[i-1],i,xin[i]);
      exit(1);
    } else
    if(xout[i] <= xout[i-1]){
...
void normalize_paired(int n, double *xin, 
  ...
  if (xn - x0 < DBL_EPSILON){
    printf("At least two distinct x values are required.\n");
    exit(1);
  }
  ...
  for(i = 1;i<n;i++){
    if((xout[i] + DBL_EPSILON) <= xout[i-1]){
      printf("X values must be non-decreasing.\n");
      printf("x[%i] = %1.20le, x[%i] = %1.20le\n",i-1,xin[i-1],i,xin[i]);
      exit(1);
    }
  }
...
void normalize_bin(int n, double *bwout, double *xin, 
  ...
  for(i=1;i<n;i++){
    if ((xin[i] - xin[(i-1)] - bw) > DBL_EPSILON){
      printf("Check the input data. The bins must have equal widths.\n");
      exit(1);
    }
  }
...
void xb(int n, int p, double *x, double *b, double *y){
  double *v,dm;
  int n2=n;
...
double normal_loglik(int n, int p, double *mu, double *y, double *sigma){
  double v = 0.0;
  int n2 = n;
  while(n2--){
    v += pow((*mu - *y),2);
...
/*
  *sigma = sqrt((v / ((double)(n - p))));
  v = -1.0 * n * log(*sigma) - 0.5 * (n - p);
*/
  *sigma = sqrt((v / ((double) n)));
  v = -1.0 * n * log(*sigma);
